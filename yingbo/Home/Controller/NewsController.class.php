...
class NewsController extends ComController {
    function showlist(){
		...
		$count = D('News')->where("is_show='0'")->count();
		...
		foreach($newsinfo as $key=>$value){
			$newsinfo[$key]['add_time'] = date("Y-m-d",$value['add_time']);
			$newsinfo[$key]['pic'] = substr($value['pic'],1);
		}
    ...
    function type(){
		...
		$count = D('News')->where("is_show='0' AND type_id = {$type_id}")->count();
...
//        dump($typeinfo);die;
		foreach($typeinfo as $key=>$value){
			$typeinfo[$key]['add_time'] = date("Y-m-d",$value['add_time']);
			$typeinfo[$key]['pic'] = substr($value['pic'],1);
		}
		$totalPages = $p->totalPages;
		if(IS_POST){
			$this->ajaxReturn(array(
