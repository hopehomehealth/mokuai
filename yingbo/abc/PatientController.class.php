...
class PatientController extends FooterController
{
    function userindex(){
        ...
        /*D('user_order')->where("shopid is not null")->delete();
        D()->execute("truncate table lh_user_suborder");
        dump(D('user_comment')->select());exit;*/
        ...
        //$res = M('user')->where("userid = 153 or userid = 92")->select();dump($res);exit;
    	$order = D('user_order');
    	$orderCount = $order ->field("status,count(orderid) as count") -> group("status") ->where("userid = {$userid}")-> select();
        $countcom = $order->where("userid = {$userid} AND is_com='1'")->count();
        $this->assign('countcom',$countcom);
        ...
        $map['shopid'] = '';
        /*判断是否已经预约过start*/
        $yuyueorder = $order ->query("select * from lh_user_order where userid = {$userid} AND shopid is null AND status = '0'");
        //dump($yuyueorder);exit;
        if($yuyueorder){
            if($yuyueorder[0]['loseeffect'] == '0'){
                $info['apointing'] = 1;
            }
        }
        /*判断是否已经预约过end*/
    ...
    function allOrder(){
    	...
    	$model = D();
        $order = D('user_order');
        $orderList1 = $order ->where("userid = {$userid} AND status = '0' AND shopid is null")->order("inputtime desc")->select();
        $orderList2 = $order ->query("select a.*,b.title from lh_user_order as a,lh_make_oredr as b where a.shopid = b.id AND a.userid = {$userid} AND status = '0' order by a.inputtime desc");
        ...
        $nurseorder = $order ->query("select a.*,b.content from lh_user_order as a,lh_make_oredr as b where a.shopid = b.id AND a.userid = {$userid} AND status != '0' AND (shopid is not null or shopid<>'') order by a.inputtime desc");
        //dump($orderList2);exit;
        foreach($nurseorder as $k => $v){
            $suborder = D()->query("select b.username,b.iphone,b.area,b.photo,b.credit,a.orderid,a.status,a.inputtime from lh_user_suborder as a,lh_user_nurse as b where a.nurseid = b.userid AND a.parent_id = {$v['orderid']} order by a.huctime");
            $count = D('user_suborder')->where("parent_id = {$v['orderid']} AND status = '2'")->count();
            if($count > 0){
                $nurseorder[$k]['finished'] = $count;
            }else{
                $nurseorder[$k]['finished'] = 0;
            }
            $nurseorder[$k]['suborder'] = $suborder;
        }
        //dump($orderList);exit;
 ...
 function loseeffect(){
        ...
        $data['loseeffect'] = '1';
        M('user_order')->where("orderid = {$orderid}")->save($data);
	...
	function del_order(){
		$order = M('user_order');
		$id = $_POST['orderid'];
		$order->where(array('orderid'=>$id))->delete();
	}
    function xinxi(){
        $daohang = array(
            'first'=>'个人信息',
    ...
    function orderinfo(){
        ...
        $model = D();
        $orderinfo = $model ->query("select a.orderid,a.inputtime,a.ordernumber,a.number,a.ssum,a.if_say,b.title,b.price,b.id from lh_user_order as a,lh_make_oredr as b where a.shopid=b.id AND a.orderid = {$orderid}");
        //$orderinfo = $order ->find($orderid);
        //$where['parent_id'] = $parent_id;
        //$where['status'] = array(neq,'2');
        $orderinfo[0]['finished'] = 0;
        if(isset($_GET['suborderid'])){
            $suborderInfo = $suborder ->find($suborderid);
            $count = $suborder->where("parent_id = {$orderid} AND status = '2'")->count();
            $orderinfo[0]['finished'] = $count;
            //$suborderInfo = $suborderList[0];
            
            
            $this ->assign('suborderInfo',$suborderInfo);
        }
       // dump($orderinfo);dump($suborderInfo);exit;
        $this ->assign('orderinfo',$orderinfo[0]);
        $this->display();
    }
    function orderauth(){
        $shenfen = $_SESSION['userInfo']['shenfen'];
        if($shenfen == 2){
        ...
        $userid = $_SESSION['userInfo']['userid'];
        $res = D('user_order')->where("userid = {$userid} AND shopid is null")->select();
        if(!$res){
            $this->ajaxReturn(array(
                    "error"=>1,
                    "content"=>'请预约评估师'
        ...
        $res2 = D('user_order')->where("userid = {$userid} AND if_say = '0' AND shopid is null")->select();
        if($res2){
            $this->ajaxReturn(array(
                    "error"=>1,
                    "content"=>'请耐心等待评估结果'
    ...
    function payorder(){
        ...
        $order = D("user_order");
        $suborder = D("user_suborder");
        $make_model = D("make_oredr");
        $model = D();
        $pay = D('pay');
        if($_POST){
            $userid = $_SESSION['userInfo']['userid'];
            
            $info = $make_model -> find($_POST['fuwuid']);
            //dump($info);exit;
            $_POST['number'] = $info['num'];
            $_POST['ssum'] = $info['price'];
            $_POST['userid'] = $userid;
            $_POST['inputtime'] = time();
            $_POST['shopid'] = $_POST['fuwuid'];
            $_POST['ordernumber'] = date('Ymd').substr(implode(NULL, array_map('ord', str_split(substr(uniqid(), 7, 13), 1))), 0, 8);
            $_POST['status'] = 0;
            //var_dump($_POST);exit;
            //$res = $order->where("orderid = 126")->select();dump($res);exit;
            $isorder = $order->where("shopid = {$_POST['fuwuid']}")->select();
            if(!$isorder){
                if($order ->create()){
                    $lastid = $order ->add();
                    if($lastid){
                        $data['parent_id'] = $lastid;
                        $data['nsum'] = round(($info['price'] / $_POST['number']),2);
                        $data['inputtime'] = $_POST['inputtime']; 
                        $data['userid'] = $userid;
                        $data['shopid'] = $_POST['fuwuid'];
        ...
        $orderinfo = $model ->query("select a.orderid,a.inputtime,a.ordernumber,a.number,a.ssum,a.if_say,b.title,b.price from lh_user_order as a,lh_make_oredr as b where a.shopid= b.id AND a.orderid = {$orderid}");
        //dump($orderinfo);exit;
    ...
    function yuyue(){
        ...
        $userid = $_SESSION['userInfo']['userid'];
        if($_POST){
            $patient_model = M('user_patient');
            $order = D('user_order');
            $patient = $patient_model->where("userid = {$userid}")->find();
            //dump($patient);exit;
    ...
    function confirm(){
        ...
        if($suborder ->where("orderid = {$suborderid}")->save($data)){
            ...
            $money = round($orderinfo['nsum'] * (100 - $basic['pct_nurse']) / 100,2);
            if($nurse ->where("userid = {$orderinfo['nurseid']}")->setInc('money',$money)){
    ...
    function pinglist(){
        ...
        $userid = $_SESSION['userInfo']['userid'];
        $model = D();
        $assessorder = $model ->field("b.username,b.iphone,b.area,b.photo,b.credit,a.if_say,a.orderid,a.status,a.inputtime")->table("lh_user_order as a,lh_user_assess as b")->where("a.auserid = b.userid AND a.userid = {$userid} AND a.is_com = '0' AND a.if_say = '1'")-> select();
        $nurseorder = $model ->field("b.username,b.iphone,b.area,b.photo,b.credit,a.if_nay,a.orderid,a.status,a.inputtime,a.parent_id")->table("lh_user_suborder as a,lh_user_nurse as b")->where("a.nurseid = b.userid AND a.userid = {$userid} AND a.is_com = '0' AND a.if_nay ='1'")-> select();
        //dump($nurseorder);exit;
        $this ->assign('assessorder',$assessorder);
        $this ->assign('nurseorder',$nurseorder);
        $this->display();
    ...
    function optable(){
        ...
        //dump($patientinfo);
        //D('user_patient')->where("userid = 1")->delete();
        
        /*M('user')->where("userid = 92")->delete();
        M('user_patient')->where("userid = 92")->delete();
        M('user_order')->where("userid = 92")->delete();
        echo 222;*/
      /*  D()->execute("truncate table lh_user");
        D()->execute("truncate table lh_user_patient");
        D()->execute("truncate table lh_user_order");
        D()->execute("truncate table lh_user_comment");
        D()->execute("truncate table lh_user_postal");
        D()->execute("truncate table lh_user_suborder");
        D()->execute("truncate table lh_user_assess");
        D()->execute("truncate table lh_user_nurse");
        echo 222;*/
    ...
    function logout(){
        ...
        unset($_SESSION['flag']);$this->redirect("User/login");
    }
}