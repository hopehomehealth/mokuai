...
class DeliveryController extends ComController {
    ...
    function address(){
        ...
        foreach($delivery as $key => $value){
            if(isset($_GET['contrlname'])){
                $delivery[$key]['link'] = SITE_URL."index.php/Home/".$_GET['contrlname'].'/'.$_GET['actionname'].'/dl_id/'.$value['dl_id'];
                cookie('contrlname',$_GET['contrlname']);
                cookie('actionname',$_GET['actionname']);
            }else if(cookie('contrlname')){
                $delivery[$key]['link'] = SITE_URL."index.php/Home/".cookie('contrlname').'/'.cookie('actionname').'/dl_id/'.$value['dl_id'];
            }else{
                $delivery[$key]['link'] = '#';
            }        
        }
