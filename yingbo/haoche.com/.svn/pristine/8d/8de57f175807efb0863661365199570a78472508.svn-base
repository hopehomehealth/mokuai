...
class UserController extends ComController {
    ...
    function bevip(){
        ...
        $userid = $_SESSION['userInfo']['userid'];
        if(($_SESSION['userInfo']['is_supvip'] == '1') && ($_SESSION['userInfo']['vipexpire'] > time())){
            $this->redirect('Union/index');
        }
        //echo 111;exit;
        $setting_model = M("setting");
        $set = $setting_model->find();
        $bevip_model = M('bevip');
        $bevip = $bevip_model->where("userid = {$userid} AND status = '0'")->find();
        $nowtime = time();
        if($bevip){
            $data['ordersn'] = 'V'.$nowtime.rand(10,99);
            $data['amount'] = $set['vipfee'];
            $data['inputtime'] = $nowtime;
            $bevip_model->where("vipid = {$bevip['vipid']}")->save($data);
        }else{
            $data['userid'] = $userid;
            $data['ordersn'] = 'V'.$nowtime.rand(10,99);
            $data['inputtime'] = $nowtime;
            $data['amount'] = $set['vipfee'];
            $bevip['vipid'] = $bevip_model->add($data);
        }

        if(isset($bevip['vipid'])){
            $url = SITE_URL.'index.php/Home/Wxpay/pay/type/vipfee/id/'.$bevip['vipid'];
        }else{
            $url = SITE_URL.'index.php/Home/Union/index';
        }
        $this->assign('url',$url);
        $this->assign('bevip',$bevip);
        $this->assign('daohang',$daohang);
        $this->display();
    }
    //用户登陆
    ...
    function userindex(){
        $daohang = array(
            'second'=>'会员中心',
            'third'=>"<if condition='$Think.session.flag eq '''><span style='position:absolute;float:right;right:0;height:4.4rem;padding-right:0.7rem'><a style='height:100%;line-height:4.4rem;color:#fff;font-size:1.5rem;' href='http://www.haoche.com/index.php/Home/User/logout'>退出</a></span></if>",
        );
        $userid = $_SESSION['userInfo']['userid'];
        $model = M();
        $user_model = M('user');
  ...
  function collect(){
    ...
    if(IS_GET){
        ...
        $detail_model = M('userdetail');
        $userdetail = $detail_model->where("userid = {$userid}")->find();
        $idArr = explode(',',ltrim($userdetail['collects'],','));
        if(in_array($seller_id,$idArr)){
            $this->ajaxReturn(array(
                'error'=>1,
                'content'=>'你已经收藏'
            ));
        }else{
            if($detail_model->where("userid = {$userid}")->setField('collects',$userdetail['collects'].','.$seller_id)){
                $this->ajaxReturn(array(
                    'error'=>0,
                    'content'=>'收藏成功'
                ));
  ...
  function collection(){
    ...
    $userid = $_SESSION['userInfo']['userid'];
    $userdetail = $detail_model->where("userid = {$userid}")->find();
    $idstr = ltrim($userdetail['collects'],',');
    if($idstr != ''){
        $map['collects'] = array('in',$idstr);
        $map['status'] = '1';
        $sellerlist = M('seller')->where($map)->select();
        $this->assign('sellerlist',$sellerlist);
        //dump($sellerlist);exit;
    }
