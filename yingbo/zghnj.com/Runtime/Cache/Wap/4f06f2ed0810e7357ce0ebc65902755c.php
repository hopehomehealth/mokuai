...
<div class="main">
    <div style="font-size:0.28rem;color:#666;padding:0 3%;">搜索到“<?php echo ((isset($count) && ($count !== ""))?($count):0); ?>”条相关记录</div>
    <?php if(!empty($result)): ?><!--*****su-con*****-->
        <section class="su-case b-white" id="scrollloading" data-scrollstatus="false" data-end="false">
            <?php if(is_array($result)): foreach($result as $key=>$info): ?><dl class="su-news" style="padding:0.1rem 0rem">
                    <?php if(($info['pic'] == './Public/Upd/news/') OR ($info['pic'] == '')): else: ?>
                        <dt><a href="/index.php/Wap/News/detail/id/<?php echo ($info["news_id"]); ?>"><img src="<?php echo (substr($info["pic"],1)); ?>"></a></dt><?php endif; ?>
                    <dd>
                        <h3><?php echo ($info["title"]); ?></h3>
                        <p><?php if(($info["datetime"] != '') OR ($info["address"] != '')): ?>时间：<?php echo ($info["datetime"]); ?><br>
                        地点：<?php echo ($info["address"]); else: echo ($info["content"]); endif; ?></p>
                        <span class="su-news-more b-bule"><a  href="<?php echo ($info["href"]); ?>">查看详情</a></span>
                    </dd>
...
<script type="text/javascript">
  function scrollLoading(scrollloading,nowpage){
    $.get("/index.php/Wap/Index/search?keywords=",{'p':nowpage},function(data){
      $("#loading").remove();
      if(data.error == 0){
        var html = '';
        for(var i in data.content){
            ...
            if((data.content[i].pic == '/Public/Upd/news/') || (data.content[i].pic == '')){          
                ...
                img = '<dt><a href="/index.php/Wap/News/detail/id/'+data.content[i].news_id+'"><img src="'+data.content[i].pic+'"></a></dt>';
            }
            var content = '';
  ...
  <div class="login-con register">
      <form action="" method="post" id="regform">
          <ul>
            ...
            <span class="login-sm"><span class="register-xz fl inputlabel"></span><span class="inputlabel">同意中国缓粘结网</span><a style="color:#207f9c;cursor:pointer">《用户使用协议》</a></span>
