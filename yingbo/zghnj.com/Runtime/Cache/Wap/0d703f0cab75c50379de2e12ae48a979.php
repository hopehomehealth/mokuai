...
<div class="main">
                ...
                <div class="probebox swiper-container-horizontal">
                    <ul class="clearfix swiper-wrapper">
                        ...
                        <?php if(is_array($foreshow)): foreach($foreshow as $key=>$info1): ?><li class="swiper-slide">
                              <a href="/index.php/Wap/News/detail/id/<?php echo ($info1["news_id"]); ?>">
                                  <img src="<?php echo (substr($info1["pic"],1)); ?>" alt="" />
    ...
    <section class="su-case b-lgray1 pt0-35" id="scrollloading" data-scrollstatus="false" data-end="false">
        ...
        <?php if(is_array($review)): foreach($review as $key=>$info2): ?><dl class="su-news-list">
              <?php if(($info2['pic'] == './Public/Upd/news/') OR ($info2['pic'] == '')): else: ?>
                <dt><a href="/index.php/Wap/News/detail/id/<?php echo ($info2["news_id"]); ?>"><img src="<?php echo (substr($info2["pic"],1)); ?>" alt=""></a></dt><?php endif; ?>
              <dd>
                  <h3><?php echo ($info2["title"]); ?></h3>
                  <p>时间：<?php echo ($info2["datetime"]); ?><br>
                      地点：<?php echo ($info2["address"]); ?></p>
                  <span class="su-news-more b-bule"><a href="/index.php/Wap/News/detail/id/<?php echo ($info2["news_id"]); ?>">查看详情</a></span>
              </dd>
...
<script type="text/javascript">
  function scrollLoading(scrollloading,nowpage){
    $.get("/index.php/Wap/News/activities/p/"+nowpage,function(data){
      $("#loading").remove();
      if(data.error == 0){
        var html = '';
        for(var i in data.content){
            ...
            html += '\
              <dl class="su-news-list">'+img+'\
                <dd>\
                    <h3>'+data.content[i].title+'</h3>\
                        <p>时间：'+data.content[i].datetime+'<br>\
                      地点：'+data.content[i].address+'\
                        <span class="su-news-more b-bule"><a href="/index.php/Wap/News/detail/id/'+data.content[i].news_id+'">查看详情</a></span>\
                    </p>\
                </dd>\
            </dl>\
            ';
