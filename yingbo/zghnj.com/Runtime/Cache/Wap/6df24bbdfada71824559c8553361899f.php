    ...
    <section class="su-con" id="scrollloading" data-scrollstatus="false" data-end="false">
        <?php if(is_array($news)): foreach($news as $key=>$info): ?><dl class="su-lear">
                <?php if(($info['pic'] == './Public/Upd/news/') OR ($info['pic'] == '')): else: ?>
                    <dt><a href="/index.php/Wap/Technology/detail/id/<?php echo ($info["news_id"]); ?>"><img src="<?php echo (substr($info["pic"],1)); ?>" alt=""></a></dt><?php endif; ?>
                <dd>
...
<script type="text/javascript">
  function scrollLoading(scrollloading,nowpage){
    $.get("/index.php/Wap/Technology/lectureRoom/p/"+nowpage,function(data){
      $("#loading").remove();
      if(data.error == 0){
        var html = '';
        for(var i in data.content){
            ...
            if((data.content[i].pic == '/Public/Upd/news/') || (data.content[i].pic == '')){          
                ...
                img = '<dt><a href="/index.php/Wap/Technology/detail/id/'+data.content[i].news_id+'"><img src="'+data.content[i].pic+'" alt=""></a></dt>';
            }
            html += '\
              <dl class="su-lear">'+img+'\
                <dd>\
