...
<div class="menu-bj">
	...
	<!----banner----><div class="banner">    <?php if(is_array($slides)): foreach($slides as $key=>$slide): ?><img src="<?php echo (substr($slide["pic"],1)); ?>" alt=""><?php endforeach; endif; ?></div><!----bannerend----><!----front----><div class="su-front">    <div class="column-a">        <ul id="0" class="su-front-tit">            <?php if(is_array($category)): foreach($category as $key=>$cateinfo): ?><li <?php if(ACTION_NAME == $cateinfo['action']): ?>class="cur-a"<?php endif; ?>><a href="/index.php/Home/<?php echo ($cateinfo['ctrl']); ?>/<?php echo ($cateinfo['action']); ?>#0"><?php echo ($cateinfo["cat_name"]); ?></a></li><?php endforeach; endif; ?>        </ul>        <div class="su-content">            <p class="su-deve"><span class="fs30 pr15">凝聚专业的缓粘结设计与施工企业</span></p>            <p class="su-deve-txt">　　缓粘结预应力技术以设计与施工为表现，以此展现缓粘结的优异性能，推动缓粘结的技术应用。</p>            <?php if(is_array($news)): $i = 0; $__LIST__ = $news;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$info1): $mod = ($i % 2 );++$i; if(($mod) == "1"): echo ($vo["name"]); ?>                <dl class="su-deve-list1">                    <dt><img src="<?php echo (substr($info1["pic"],1)); ?>" alt=""></dt>                    <dd>                        <h3><?php echo ($info1["title"]); ?></h3>                        　　<?php echo ($info1["content"]); ?>                    </dd>                </dl><?php endif; endforeach; endif; else: echo "" ;endif; ?>            <div class="su-deve-list2-bj">                <?php if(is_array($news)): $i = 0; $__LIST__ = $news;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$info2): $mod = ($i % 2 );++$i; if(($mod) == "0"): echo ($vo["name"]); ?>                    <dl class="su-deve-list2">                        <dd>                            <h3><?php echo ($info2["title"]); ?></h3>                            　　<?php echo ($info2["content"]); ?>                        </dd>                        <dt><img src="<?php echo (substr($info2["pic"],1)); ?>" alt=""></dt>                    </dl><?php endif; endforeach; endif; else: echo "" ;endif; ?>            </div>            <p class="su-deve-tit">成员企业</p>            <ul class="su-deve-img">                <li>                    <img src="<?php echo (HIMG_URL); ?>images_57.jpg" alt=""><br>                    触控行业                </li>                <li>                    <img src="<?php echo (HIMG_URL); ?>images_58.jpg" alt=""><br>                    行业综合服务商                </li>                <li>                    <img src="<?php echo (HIMG_URL); ?>images_59.jpg" alt=""><br>                    功能胶带                </li>            </ul>        </div>    </div></div><!----frontend---->
<!--footer-->
<div class="footer">
    <div class="column">
        <ul class="sumenu fl">
