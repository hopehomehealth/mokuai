            ...
            <div class="page-content box">
                <div class="box-title margin_bot_20">
                    <div class="span10">
                        ...
                        <div class="pull-right"><a class="btn btn-success btn-sm" href="/index.php/Admin/News/tianjia">发布文章</a></div>
                    </div>
                </div>
                <form class="form-horizontal"  action="/index.php/Admin/News/showlist" method="GET">
                    <div class="form-group">
                        <div>
                            <span class="shuaixuanbtn" style="line-height:1.5;color:block;font-weight:bold;border:none">标题 : </span>
                            <input type="text" class="shuaixuanbtn" placeholder="" value="<?php echo ($_GET['search']); ?>" name="search">
                            ...
                            <select name="cat_id"  class="shuaixuanbtn" style="padding-right: 4px;">
                                ...
                                <option value='0' style="color:black" <?php if(($_GET['cat_id'] == 0) AND ($_GET['cat_id'] != '')): ?>selected<?php endif; ?>>　　关注热点</option>
                ...
                <table class="table table-striped table-bordered table-hover">
                    ...
                    <?php if(is_array($info)): foreach($info as $k=>$v): ?><tr>
                            <td><input type="checkbox" name="news_id[]" value="<?php echo ($v["news_id"]); ?>"></td>
                            <td><div align="center"><?php echo ($v["news_id"]); ?></div></td>
                            <td><div align="center"><a href="<?php echo U('News/showlist',array('cat_id'=>$v['cat_id']));?>"> <?php echo ($v["cat_name"]); ?></a></div></td>
                            <td><div align="center"><?php echo ($v["title"]); ?></div></td>
                            <td><div align="center"><?php echo ($v["author"]); ?></div></td>
                            <td><div align="center"><img src="<?php echo (SITE_URL); echo ($v["pic"]); ?>" onerror="this.src='<?php echo (IMG_URL); ?>imgerror.png'" width='70' height='50'/></div></td>
                            ...
                            <td> <div align="center">
                                <div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">
                                    <a class="green" href="<?php echo U('upd',array('news_id'=>$v['news_id'],'nowpage'=>$_GET['p']?$_GET['p']:1));?>"> <i class="icon-pencil bigger-130"></i>&nbsp;编辑 </a>
                                    <a class="red detail" href="<?php echo U('del',array('news_id'=>$v['news_id']));?>"> <i class="icon-trash bigger-130"></i>&nbsp;删除 </a>
                                </div>
                            </div>
                            </td>
    ...
    $(function() {
        ...
        $("#zhanshi").click(function () {
            if ($("input[name='news_id[]']:checked").length >= 1) {
                $("#batches").attr('action', "/index.php/Admin/News/zhanshi/nowpage/<?php echo ((isset($_GET['p']) && ($_GET['p'] !== ""))?($_GET['p']):1); ?>");
                $("#batches").submit();
            } else {
                alert('请选择操作对象');
            }
        ...
        $("#yincang").click(function () {
            if ($("input[name='news_id[]']:checked").length >= 1) {
                $("#batches").attr('action', "/index.php/Admin/News/yincang/nowpage/<?php echo ((isset($_GET['p']) && ($_GET['p'] !== ""))?($_GET['p']):1); ?>");
                $("#batches").submit();
            } else {
                alert('请选择操作对象');
            }
        ...
        $("#shanchu").click(function () {
            if ($("input[name='news_id[]']:checked").length >= 1) {
                if (confirm('你确定要永久删除吗')) {
                    $("#batches").attr('action', "/index.php/Admin/News/del/nowpage/<?php echo ((isset($_GET['p']) && ($_GET['p'] !== ""))?($_GET['p']):1); ?>");
                    $("#batches").submit();
                }
            } else {
                alert('请选择操作对象');
