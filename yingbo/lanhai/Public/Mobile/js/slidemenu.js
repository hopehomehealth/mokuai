$(function(){
    window.Swipe = function(b, a) {
	    ...
	    this.col=this.options.auto || 5;//每排显示个数
        this.container = b;
    ...
    Swipe.prototype = {
        setup: function() {
            ...
            this.width = Math.ceil(("getBoundingClientRect" in this.container) ? 
        ...
        slide: function(a, c) {
            ...
            b.webkitTransitionDuration = b.MozTransitionDuration = 
                b.msTransitionDuration = b.OTransitionDuration = 
                b.transitionDuration = c + "ms";
            b.MozTransform = b.webkitTransform = 
                "translate3d(" + -(a * this.width) + "px,0,0)";
            b.msTransform = b.OTransform = 
                "translateX(" + -(a * this.width) + "px)";
            this.index = a
        },
        getPos: function() {
            return this.index
        ...
        onTouchMove: function(a) {
            ...
            if (!this.isScrolling) {
                ...
                this.deltaX = this.deltaX / ((!this.index && this.deltaX > 0 ||
                    ...
                    (Math.abs(this.deltaX) / this.width + 1) : 1);
                this.element.style.MozTransform = 
                    this.element.style.webkitTransform = 
                    "translate3d(" + (this.deltaX - this.index * this.width) +
                        "px,0,0)";
                a.stopPropagation()
            }
        },
        onTouchEnd: function(c) {
            var b = Number(new Date()) - this.start.time < 250 && 
                Math.abs(this.deltaX) > 20 || Math.abs(this.deltaX) > 
                this.width / 2,
            a = !this.index && this.deltaX > 0 || this.index == 
                this.length - 1 && this.deltaX < 0;
            if (!this.isScrolling) {
                this.slide(this.index + (b && !a ? 
                    (this.deltaX < 0 ? 1: -1) : 0), this.speed)
            }
            c.stopPropagation()
        }
    };
    ...
    var slider=new Swipe(document.getElementById('menu'),
        {speed:300,auto:0,width:80,col:5});
