	...
	public function findpwd() {
		if(IS_POST){
			...
			if ($userinfo) {
				...
				$findpwd = $_SERVER ['HTTP_HOST'] . U ( 'Email/newpassword' ) . '?key=' . $key . '&info=' . $string . '&time='.$time.'&code=' .$code; // code是用来检验time是否有修改过

				// 调用发送邮件函数
				$title="找回密码(蓝海长青)";

				$content="<h3>亲爱的蓝海长青会员：$username 用户</h3><br/><br/><br/>请点击下面链接找回密码<br><br>http://{$findpwd}<br><br><br><h4>有效期30分钟</h4><br><br>";
