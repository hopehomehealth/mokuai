  ...
  <div class="for-c fl">
    <div class="for-con cur" id="recordslist">
      <?php if(is_array($postlist)): foreach($postlist as $key=>$info): ?><dl class="for-list fl">
        <dt></dt>
        <dd style="width:97%">
          <p class="w100 t-w pb3-5" style="padding-bottom:2.5rem"><a style="float:left;height:3rem;overflow:hidden;width:89%;text-overflow: ellipsis;" href="/index.php/Mobile/Posts/detail/post_id/<?php echo ($info["post_id"]); ?>"><?php if($info["sort"] == 3): ?><span class="zhiding">置顶</span><?php elseif($info["sort"] == 2): ?><span class="jinghua">精华</span><?php elseif($info["sort"] == 1): ?><span class="xintie">新帖</span><?php elseif($info["sort"] == 0): ?><span class="putong">普通</span><?php endif; ?>&nbsp;<?php echo ($info["topic"]); ?></a><a class='fr cancelcoll' style='height:3rem;width:2.5rem;background:url(/Public/Home/images/images_165.jpg) no-repeat center' href="/index.php/Mobile/Posts/cancelcoll/post_id/<?php echo ($info["post_id"]); ?>"></a></p>
      ...
      $.get("/index.php/Mobile/User/mycollect",{'p':nowpage},function(data){
        if(data.error == 0){
          ...
          for(var i in data.content){
            ...
            html += "<dl class='for-list fl'><dt></dt><dd style='width:97%'><p class='w100 t-w pb3-5' style='padding-bottom:2.5rem'><a style='float:left;height:3rem;overflow:hidden;width:89%;text-overflow: ellipsis;' href='/index.php/Mobile/Posts/detail/post_id/"+data.content[i].post_id+"'>"+status+"&nbsp;"+data.content[i].topic+"</a><a class='fr cancelcoll' style='height:3rem;width:2.5rem;background:url(/Public/Home/images/images_165.jpg) no-repeat center' href='/index.php/Mobile/Posts/cancelcoll/post_id/"+data.content[i].post_id+"'></a></p><p class='w100 fl'><span class='fl w30 t-w'>"+data.content[i].username+"</span><span class='fr pl1'>"+data.content[i].ctime+"</span><span class='fr for-list-tb'></span> </p></dd></dl>";
          }
        }else{
          if(!haveend){
