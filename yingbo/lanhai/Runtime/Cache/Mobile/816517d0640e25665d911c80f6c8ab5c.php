...
<p class="xj-hdts fl pl0-5 pt0-5"><img src="<?php echo (MIMG_URL); ?>qianxleft1.png"></p> <p class="xj-hdts fr pr0-5 pt0-5"><img src="<?php echo (MIMG_URL); ?>qianxr1.png"></p>
	<div id="menu">
		<ul class="slide">
			...
			<li class="current"><a href="/index.php/Mobile/Blog/myclass">分类管理</a></li>
	...
	<div class="settmain fenlei show">
		<form action="/index.php/Mobile/Blog/myclass" method="post" id="addclassform">
		<table>
			<tr>
		        <td style="width:40%;text-align:right;border:none;" colspan=2><input type="text" name="class_name" value="" style="width:80%; float:left;" placeholder="请输入分类名" class="webkitinput"></td>
		        <td style="width:33.3%;border:none; padding-bottom:0.5rem;"><button style="padding:0.3rem 0.5rem;background-color: #004084;color: #ffffff;font-size:1.375rem;-moz-border-radius: 0.25rem;-webkit-border-radius: 0.25rem;border-radius: 0.25rem;" type="button" onclick="subclassform()">添加</button></td>
				<td></td>
			</tr>
		</table>
		</form>
		<table id="classtable" style="width:100%;text-align:left">
			<thead>
			<tr>
				<td>分类</td>
				<td>博文数</td>
			...
			<tbody>
			<?php if(is_array($classlist)): foreach($classlist as $key=>$classinfo): ?><tr>
				<td><?php echo ($classinfo["class_name"]); ?></td>
				<td><?php echo ($classinfo["number"]); ?></td>
				<td><a href='javascript:void(0)' style="padding-right:0;" classid="<?php echo ($classinfo["class_id"]); ?>" class='delclass' onclick="delclass(this,<?php echo ($classinfo["class_id"]); ?>)">删除</a></td>
