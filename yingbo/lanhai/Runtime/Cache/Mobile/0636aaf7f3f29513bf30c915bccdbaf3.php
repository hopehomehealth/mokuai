    ...
    <div class="for-con cur" id="recordslist" boardid="<?php echo ($_GET['board_id']); ?>">
      <?php if(is_array($postlist)): foreach($postlist as $key=>$info): ?><dl class="for-list fl">
        ...
        <dd style="width:97%">
          ...
          <p class="w100 fl"><span class="fl w30 t-w"><?php echo ($info["username"]); ?></span><span class="fl pl1"><?php echo (date("Y-m-d H:s",$info["ctime"])); ?></span><span class="fr for-list-tb"></span> </p>
        </dd>
...
<script type="text/javascript">
  $(function(){
    ...
    function ajax_request(boardid,nowpage){
      ...
      $.get("/index.php/Mobile/Posts/postlist/board_id/"+boardid,{'p':nowpage},function(data){
        if(data.error == 0){
          var status = '';
          for(var i in data.content){
            ...
            html += "<dl class='for-list fl'><dt></dt><dd style='width:97%'><p class='w100 t-w pb3-5' style='padding-bottom:2.5rem'><a href='/index.php/Mobile/Posts/detail/post_id/"+data.content[i].post_id+"'>"+status+"&nbsp;"+data.content[i].topic+"</a></p><p class='w100 fl'><span class='fl w30 t-w'>"+data.content[i].username+"</span><span class='fl pl1'>"+data.content[i].nctime+"</span><span class='fr for-list-tb'></span> </p></dd></dl>";
          }
        }else{
          if(!haveend){
