...
<p class="xj-hdts fl pl0-5 pt0-5"><img src="<?php echo (MIMG_URL); ?>qianxleft1.png"></p> <p class="xj-hdts fr pr0-5 pt0-5"><img src="<?php echo (MIMG_URL); ?>qianxr1.png"></p>
	<div id="menu">
		<ul class="slide">
			...
			<li class="current"><a href="/index.php/Mobile/Blog/myarticles">博文管理</a></li>
	...
	<div class="settmain manage show">
		<form action="/index.php/Mobile/Blog/myarticles" method="get">
			<label for="classify">
				<span>博文分类：</span>
				<select name="class_id" id="classify" style="outline:none">
					<option style="font-size:1rem;outline:none" value="0">全部分类</option>
					<?php if(is_array($classlist)): foreach($classlist as $key=>$classinfo): ?><option style="font-size:1rem;outline:none" value="<?php echo ($classinfo['class_id']); ?>"><?php echo ($classinfo['class_name']); ?></option><?php endforeach; endif; ?>
				</select>
			</label>
			<label for="keyword">
				<span>关&nbsp;键&nbsp;字&nbsp;：</span>
		...
		<table>
			...
			<tbody>
			<?php if(is_array($articlelist)): foreach($articlelist as $key=>$artinfo): ?><tr>
				<td>
					<h4><?php echo ($artinfo["title"]); ?></h4>
					<time><?php echo (date("Y/m/d H:i",$artinfo["add_time"])); ?></time>
				</td>
				<td><?php echo ($artinfo["views"]); ?></td>
				<td><?php echo ($artinfo["comments"]); ?></td>
				<td>
					<a href="/index.php/Mobile/Blog/editart/art_id/<?php echo ($artinfo["art_id"]); ?>">编辑</a><a href="/index.php/Mobile/Blog/delarticle/art_id/<?php echo ($artinfo["art_id"]); ?>" class="delarticle">删除</a>
				</td>
