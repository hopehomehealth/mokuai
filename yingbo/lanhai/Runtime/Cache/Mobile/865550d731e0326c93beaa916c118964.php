    ...
    <section>
        <div class="nht-xl fl">
            ...
            <ul class="nht-sumenu fr">
              <?php if(is_array($guanyu2)): foreach($guanyu2 as $key=>$v): if(($v["url"] == '') AND ($v["lan_id"] == 10)): ?><li><a href="<?php echo U('News/guanyuwomen',array('lan_id'=>2,'zilanid'=>$v['lan_id']));?>"><?php echo ($v["lan_title"]); ?></a></li>

				<?php elseif(($v["url"] == '') AND ($v["lan_id"] == 12)): ?>
<li><a href="<?php echo U('News/guanyuwomen',array('lan_id'=>2,'zilanid'=>$v['lan_id']));?>"><?php echo ($v["lan_title"]); ?></a></li>
				<?php elseif(($v["url"] == '') AND ($v["lan_id"] == 13)): ?>
<li><a href="<?php echo U('News/guanyuwomen',array('lan_id'=>2,'zilanid'=>$v['lan_id']));?>"><?php echo ($v["lan_title"]); ?></a></li>
				<?php elseif(($v["url"] == '') AND ($v["lan_id"] == 14)): ?>
<li><a href="<?php echo U('News/guanyuwomen',array('lan_id'=>2,'zilanid'=>$v['lan_id']));?>"><?php echo ($v["lan_title"]); ?></a></li>
...
<li><a href="<?php echo ($v["url"]); ?>"><?php echo ($v["lan_title"]); ?></a></li><?php endif; endforeach; endif; ?>
    ...
    <!--*****banner*****-->
    <section>
        <p class="abo-banner fl">   <?php if(is_array($adinfo)): foreach($adinfo as $key=>$v): ?><img src="<?php echo (SITE_URL); echo ($v["pic_big"]); ?>"><?php endforeach; endif; ?></p>
    </section>
    <!--*****h-l内容*****-->
    <style>
        ...
        .v01{float:right;width:1.4rem;height:1.4rem;overflow:hidden;display:inline-block;margin-top:-0.5rem;margin-bottom:-0.5rem;background:url("<?php echo (MIMG_URL); ?>vicon.png") no-repeat right;background-size:30%;}
        .v02{float:right;width:1.4rem;height:1.4rem;overflow:hidden;display:inline-block;margin-top:-0.5rem;margin-bottom:-0.5rem;background:url("<?php echo (MIMG_URL); ?>vicon1.png")no-repeat right;background-size:60%;}
        .vtitle{width:100%;height:3.5rem;line-height:3.5rem;margin-top:-1px; border-bottom: 1px #004084  solid;font-size:1.5rem;color:#4d4d4d;;cursor:pointer;}
    ...
    <section>
		...
		<?php if(is_array($catinfo)): foreach($catinfo as $k=>$v): ?><input type="hidden" id="<?php echo ($v["lan_id"]); ?>" value="<?php echo ($v["lan_id"]); ?>">
            <div class="vtitle fl"><em <?php if($v["lan_id"] == $_GET['zilanid']): ?>class="v01 v02"<?php else: ?>class="v01"<?php endif; ?>></em><span class="lin-tt"><?php echo ($v["lan_title"]); ?></span></div>
			<?php if(is_array($v["info"])): foreach($v["info"] as $kk=>$vv): ?><div <?php if($v["lan_id"] == $_GET['zilanid']): ?>class="vcon"<?php else: ?>class="vcon" style="display: none;"<?php endif; ?>>
                <div class="abo-nr fl clearfix">
			  ...
			  <span class="fwb fl" style="padding:1rem 0rem;font-size:1.5rem;">公司电话：010-83671790</span><br>
                    <span class="fwb fl" style="padding:0rem 0rem 1rem 0rem;font-size:1.5rem;">公司地址：北京市海淀区田村北路墨蔬院06号</span>
          <p>
                <div id="l-map"></div>
                <div id="r-result"></div>
                </p>
	 <script type="text/javascript">
        // 百度地图API功能
         sContent ="公司地址：北京市海淀区田村北路墨蔬院06号";
