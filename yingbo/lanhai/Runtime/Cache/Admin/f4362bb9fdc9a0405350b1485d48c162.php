        ...
        <div class="row">
            <div class="page-content box">
                <div class="box-title margin_bot_20">
                    <div class="span10">
                        <a href="<?php echo U('showlist');?>"><h3><i class="icon-magic"></i>友情列表</h3></a>
                    </div>
                </div>

                <form class="form-horizontal"  action="/index.php/Admin/Link/showlist" method="post">
                    <div class="form-group">
						<label class="control-label" style="width:50px">类型 : </label>
                        <div class="controls pull-left">
                            <select name="cat_id">
								<?php if(is_array($lcats)): foreach($lcats as $key=>$catinfo): if($catinfo['cat_id'] == $catid): ?><option value="<?php echo ($catinfo["cat_id"]); ?>" selected="selected"><?php echo ($catinfo["cat_name"]); ?></option>
								<?php else: ?>
									<option value="<?php echo ($catinfo["cat_id"]); ?>"><?php echo ($catinfo["cat_name"]); ?></option><?php endif; endforeach; endif; ?>
							</select>
                        </div>
                        ...
                        <div class="margin_bot_20" align="right">
                            <a href="<?php echo U('Link/tianjia');?>" class="btn btn-primary btn-sm"><i class="icon-plus"></i>友情链接添加</a>
                ...
                <table class="table table-striped table-bordered table-hover">
                    ...
                    <?php if(is_array($info)): foreach($info as $k=>$v): ?><tr>
                            <td><div align="center"><input type="checkbox" name="link_id[]" value="<?php echo ($v["link_id"]); ?>"></div></td>
                            <td><div align="center"><?php echo ($v["link_id"]); ?></div></td>
                            <td><div align="center"><?php echo ($v["link_name"]); ?></div></td>
                            <td><div align="center"><img src="<?php echo (SITE_URL); echo ($v["pic"]); ?>"  width='70' height='50'/></div></td>
                            <td><div align="center"><a href="<?php echo U('Link/typelist',array('cat_id'=>$v['cat_id']));?>"><?php echo ($v["cat_name"]); ?></a> </div></td>
                            <?php if($v["is_no"] == 0): ?><td><div align="center"><i class="icon-ok bigger-130 green"></i></div> </td>
                                <?php else: ?>
                            ...
                            <?php if($v["is_show"] == 0): ?><td><div align="center"><i class="icon-ok bigger-130 green"></i></div> </td>
                                <?php else: ?>
                                <td><div align="center"><i class="icon-remove bigger-130 red"></i></div> </td><?php endif; ?>
							<td><div align="center"><input style="width:100%;text-align:center" linkid="<?php echo ($v["link_id"]); ?>" type="text" name="sort" value="<?php echo ($v["sort"]); ?>"></div></td>
                            ...
                            <td> <div align="center">
                                <div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">
                                    <a class="green" href="<?php echo U('upd',array('link_id'=>$v['link_id'],'p'=>$p));?>"> <i class="icon-pencil bigger-130"></i>&nbsp;编辑 </a>
                                    <!--<a onclick="return confirm('确定要删除吗')" href="<?php echo U('del',array('link_id'=>$v['link_id']));?>" class="red detail"> <i class="icon-remove bigger-130"></i>&nbsp;删除 </a>-->
                                </div>
                            </div>
                            </td>
...
<script type="text/javascript">
    $(function() {
        ...
        $("#zhanshi").click(function () {
            if ($("input[name='link_id[]']:checked").length >= 1) {
                $("#batches").attr('action', "/index.php/Admin/Link/zhanshi");
                $("#batches").submit();
            } else {
                alert('请选择操作对象');
            }
        ...
        $("#yincang").click(function () {
            if ($("input[name='link_id[]']:checked").length >= 1) {
                $("#batches").attr('action', "/index.php/Admin/Link/yincang");
                $("#batches").submit();
            } else {
                alert('请选择操作对象');
            }
        ...
        $("#shanchu").click(function () {
            if ($("input[name='link_id[]']:checked").length >= 1) {
                if (confirm('你确定要永久删除吗')) {
                    $("#batches").attr('action', "/index.php/Admin/Link/del");
                    $("#batches").submit();
                }
            } else {
                alert('请选择操作对象');
	...
	$("input[name='sort']").blur(function(){
		...
		$.get("/index.php/Admin/Link/sortlink",{'sort':$(this).val(),'link_id':$(this).attr('linkid')},function(data){
