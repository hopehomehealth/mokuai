...
<a href="<?php echo ($v["url"]); ?>"><?php echo ($v["lan_title"]); ?></a><?php endif; endforeach; endif; ?>
        ...
        <li <?php if((ACTION_NAME == lanhaizixun) OR ($lan_id == 18) OR ($lan_id == 19) OR ($lan_id == 20) OR ($lan_id == 21) OR ($lan_id == 22) OR ($lan_id == 23)): ?>class="cur"<?php endif; ?>>            <a href="<?php echo U('News/lanhaizixun');?>"><?php echo ($lanhaizixun1[0]['lan_title']); ?></a>
            <div class="childnavin">
                <img src="<?php echo (HIMG_URL); ?>images_01.png" />
...
<ul style="width: 1000px;margin:0 auto;background: url(<?php echo (HIMG_URL); ?>images_10.png) no-repeat left bottom;line-height: 36px;height:36px;display:block;margin-top:-36px;z-index:999;position:relative">
  <li style="float:left;"><a style="color:#fff;height:36px" href="/index.php/Home/Blog/myblog">设置管理</a></li>
...
<div class="column-c pt10 pb30">
  ...
  <div class="set-lis-l fl">
    ...
    <form action="/index.php/Home/Blog/editart/art_id/65" method="post" id="articleform2">
     <ul class="advice fl">
      <li>
          <p class="poster-t fl">标题：</p>
          <input name="title" class="advice-k w520 fl" placeholder="请输入文章标题！" type="text" value="<?php echo ($artinfo["title"]); ?>" />
          <select class="poster-k fl ml28" name="class_id">
            <option style="color:#666;" value='0'>&nbsp;&nbsp;&nbsp;&nbsp;选择分类</option>
            <?php if(is_array($classlist)): foreach($classlist as $key=>$classinfo): ?><option style="color:#666;" value='<?php echo ($classinfo["class_id"]); ?>' <?php if($classinfo['class_id'] == $artinfo['class_id']): ?>selected="selected"<?php endif; ?>>&nbsp;&nbsp;&nbsp;&nbsp;<?php echo ($classinfo["class_name"]); ?></option><?php endforeach; endif; ?>
          </select>
          </p>
        </li>
        <li>
          <p class="poster-t fl">内容：</p> <textarea rows="5" cols="25" id='content' name="content" class="advice-k w653 fl" style='padding:0;border:0;width:663px;height:455px;'><?php echo ($artinfo["content"]); ?></textarea><div style="clear:both"></div>
        </li>
        <script type="text/javascript">
      ...
      <li>
        <p class="poster-t fl">&nbsp;</p><p class="lh34"><span class=" pr20"> <!--<label>
                  <input type="radio" name="RadioGroup1" value="博文置顶" id="RadioGroup1_0" />
                  博文置顶</label>-->
               请遵守<a href="" class="bule">蓝海长青公约</a>言论规则，不得违反国家法律法规</span> </p>
      </li>
      <li>
		<input type="hidden" name="art_id" value="<?php echo ($artinfo["art_id"]); ?>">
        <span class="logon-an fr mr15"><a href="javascript:void(0)" onclick="subarticleform2()">保&nbsp;存</a></span>
      </li>
    </ul>
    </form>
  </div>
