...
<!--banner-->
<!--第二通栏-->
<div class="column-c pt10 pb30" style="height:auto;min-height:870px">
  <!--****左边通栏****-->
  <div class="set-lis-l fl h810" style="height:auto;min-height:810px;margin-bottom:20px">
    <!--****左边第一通栏****-->
  <form action="/index.php/Home/Posts/createpost" method="post" id="postform">
   <ul class="advice fl">
  <li>
          <p class="poster-t fl">标题：</p>
          <input name="topic" class="advice-k w520 fl" placeholder="请输入帖子标题！" type="text" />
          <select class="poster-k fl ml28" name="board_id">
            <option style="color:#666;" value='0'>请选择分类</option>
            <?php if(is_array($boards)): foreach($boards as $key=>$bd): if($bd["pid"] == 0): ?><option style="color:#dcc;font-weight:bold" disabled="disabled" value='0'><?php echo ($bd["board_title"]); ?></option>
              <?php else: ?>
                  <option style="color:#666;" value='<?php echo ($bd["board_id"]); ?>'>&nbsp;&nbsp;&nbsp;&nbsp;<?php echo ($bd["board_title"]); ?></option><?php endif; endforeach; endif; ?>
          </select>
          </p>
        </li>
        <li><p class="poster-t fl">内容：</p> <textarea rows="5" cols="25" id='content' name="body" class="w653 fl" style='padding:0;border:0;width:663px;height:445px;'></textarea><div style="clear:both"></div></li>
        <script type="text/javascript">
      ...
      <li><p class="poster-t fl">&nbsp;</p><p class="lh34">请遵守<a href="/index.php/Home/Sysconfig/gongyue" class="bule">蓝海长青公约</a>言论规则，不得违反国家法律法规</span> </p>
      </li>
      <li> <p class="poster-t fl">&nbsp;</p> <input name="verifycode" type="text" class="logon-list-k1 fl mr15" placeholder="请输入验证码"  />
        <img src="/index.php/Home/Posts/verifyImg" onclick="this.src=this.src+'?'+Math.random()" /><span class="logon-an fr mr15"><a href="javascript:void(0)" id="postbtn">发&nbsp;表</a></span>
      </li>
  </ul>
  </form>
  </div>
