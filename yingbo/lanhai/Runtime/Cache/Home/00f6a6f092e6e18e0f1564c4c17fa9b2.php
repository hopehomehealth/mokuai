...
<div class="column-c pt10 ser pb30">
  ...
  <ul class="bus-a-l fl">
  <?php if(is_array($catinfo)): foreach($catinfo as $k=>$v): ?><li <?php if($cat_id == $v['cat_id']): ?>class="cur"<?php endif; ?>href="<?php echo U('falvfagui',array('cat_id'=>$v['cat_id']));?>"><?php echo ($v["name"]); ?></li><?php endforeach; endif; ?>
  </ul>
    <?php if(is_array($catinfo)): foreach($catinfo as $k=>$v): ?><ul <?php if($cat_id == $v['cat_id']): ?>class="bus-a-r tab-acheck fr setting"<?php else: ?>class="bus-a-r fr setting" style="display:none;"<?php endif; ?>>
    ...
    $.get(url,function(data){
        if(data.error == 0){
            ...
            for(var i in data.fagui){
                ...
                posthtml +="<li><p class='set-lis-wz fl' style='width:75%;'><a href='<?php echo (SITE_URL); ?>/index.php/Home/News/detail/news_id/"+data.fagui[i]['news_id']+"/lan_id/"+data.fagui[i]['lan_id']+"'>"+data.fagui[i]['news_title']+"</a></p><span class='set-lis-plgz fr fs12'>"+data.fagui[i]['add_time']+"</span></li>";
            }
