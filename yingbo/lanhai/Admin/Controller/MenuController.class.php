...
class menuController extends AdminController
{
    ...
    function tianjia(){
        if(IS_POST){
           $menu = D('menu');
		   $menuinfo = $menu->find($_POST['pid']);
		   if($_POST['pid'] == 0){
			$_POST['path'] = '';
		   }else{
			$_POST['path'] = $menuinfo['path'].'/'.$menuinfo['sort'];
		   }
		   if(!isset($_POST['sort'])){
			$_POST['sort'] = 1;
		   }
    ...
    function upd(){
        ...
        if(IS_POST){
			if(!isset($_POST['pid'])){
				//$_POST['pid'] = 0;
				$menuinfo = $menu->find($_POST['c_id']);

				$menus = $menu->field("group_concat(id) as ids")->where("pid = {$menuinfo['id']}")->select();

				if($menu->where(array("id"=>$_POST['c_id']))->save($_POST)){
					$map['id'] = array('in',$menus['ids']);
					if($menus['ids'] == ''){
						$map['id'] = array('in','0');
					}
					$menu->where($map)->setField('path',$menuinfo['path'].'/'.$menuinfo['sort']);
	...
	public function  class_send(){
		//读取appid
		$model = M('sysconfig');
		$sysconfig = $model->find();

		if (!$sysconfig['appid']||!$sysconfig['appsecret']){
			$this->error('您未设置开发者凭据appid和appsecret值，无法生成自定义菜单',U('menu/showlist'));
		}else{
			if(IS_GET){
				//dump($api);
				$url_get='https://api.weixin.qq.com/cgi-bin/token?grant_type=client_credential&appid='.$sysconfig['appid'].'&secret='.$sysconfig['appsecret'];
