...
$this->assign('pid1',$pid1);
		...
		$title = $firstname.'_'.$secondname.'_'.'蓝海长青';
		$this->assign('title',$title);
        //排行
        if($pid == 0){
			//$first_url = "http://www.bjlhcq.com/index.php/Home/News/newslist/lan_id/".$lan_id;
			 $daohang=array(
                'first'=>$firstname,
                'second'=>$secondname,
			...
			if($pid == 4){
			$second_url = U('lanhaizixun');
			}else if($pid == 5){
			$second_url = U('junminronghe');
			}else if($pid == 6){
			$second_url = U('zixunfuwu');
			}
...
$title = $info['news_title'].'_'.$firstname.'_'.'蓝海长青';
$this->assign('title',$title);
        $this->display();
    }
