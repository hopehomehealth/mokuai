...
class TeamAssessController extends AdminController
{
       //列表
    function showlist(){
        if($_POST){
    ...
    function upd(){
        ...
        $team = D('TeamAssess');
        if(IS_POST) {
            $info = $team->where(array('team_id'=>$team_id))->create($team_id);
            $info['input_time'] = time();
    ...
    function del(){
       ...
       if(D('TeamAssess')->delete($team_id)){
