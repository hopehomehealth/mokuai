...
function setA(){
	...
	for(var i=0;i<arr.length;i++){
		...
		var datearr=date.split("-");
			...
			arr[i].setAttribute("onclick","javascript:setRigth("+datearr[0]+","+datearr[1]+","+datearr[2]+");javascript:personalList("+datearr[0]+","+datearr[1]+","+datearr[2]+");javascript:stopBubble(this);");
			}
...
function personalList(year,month,day){
	var url =window.location.href;
	$.post(url,{'year':year,'month':month,'day':day},function(data){
		if(data.error == 1){
			$("#aboluo-personallist").html('');
			$("#aboluo-personallist").append("<span style='display:block;margin:50px auto;text-align:center;color:red;font-size:20px'>"+data.content+"</span>");
		}else{
			//var html ='';
			$("#aboluo-personallist").html('');
			for(var i=0;i<data.content.length;i++){
				$("#aboluo-personallist").append("<div class='base-info'><a href=''><span> ID ："+data.content[i].id+"</span><span>姓名：<b style='font-size:15px;color:white'>"+data.content[i].username+"</b></span><span>电话："+data.content[i].iphone+"</span><span>地址："+data.content[i].area+"</span></a></div>");
			}
		}
	},"json");
}