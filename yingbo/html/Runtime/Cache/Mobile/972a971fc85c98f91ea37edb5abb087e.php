...
<div id="box" class="indexBox">
    <header>
        ...
        <div class="main">
             <div class="listNav">
                <ul class="listNavIn">
                    <?php if(is_array($catinfo)): foreach($catinfo as $kc=>$class): ?><li <?php if($kc == 0): ?>class="cur"<?php else: endif; ?>><?php echo ($class["name"]); ?></li><?php endforeach; endif; ?>
                </ul>
            </div>
            <div class="listNavBox">
            <?php if(is_array($catinfo)): foreach($catinfo as $k=>$v): ?><ul <?php if($k == 0): ?>class="Navkind cur"<?php else: ?>class="Navkind"<?php endif; ?>>
                            <?php if(is_array($v["productinfo"])): foreach($v["productinfo"] as $kk=>$vv): ?><li>
                                        <a href="<?php echo U('Product/detail',array('pro_id'=>$vv['pro_id']));?>">
                                            <img src="<?php echo (SITE_URL); echo (substr($vv["pic"],2)); ?>">
                                            <span><?php echo (mb_substr(str_replace(array(" ","　","t","n","r"),array("","","","",""),strip_tags($vv["china"])),0,6,'utf-8')); ?></span>
                                        </a>
    ...
    <script>        
     $(function(){
        $(".listNavIn li").click(function(){
            $(this).addClass("cur").siblings().removeClass("cur");
            $(".Navkind").eq($(this).index()).addClass("cur").siblings().removeClass("cur");
        })
     })
    </script>
