        ...
        <div class="row">
            <div class="page-content box">
                <div class="box-title margin_bot_20">
                    <div class="span10">
                        <h3><i class="icon-cogs"></i>评论列表</h3>
                    </div>
                </div>
                <table class="table table-striped table-bordered table-hover">
                    <thead>
                    <tr><th width="50px" >编号</th>
                        <th width="100px">被评论用户</th>
                        <th width="100px">手机号</th>
                        <th width="100px">评论数量</th>
                        <th width="100px">评价星级</th>
                        <th width="100px">操作</th>
                    </tr></thead>
                    <tbody>
                    <?php if(is_array($commentList)): foreach($commentList as $k=>$v): ?><tr>
                            <td><?php echo ($firstno++); ?></td>
                            <td><?php echo ($v["user"]); ?>(<?php if($v["shenfen"] == '2'): ?>评估师<?php endif; if($v["shenfen"] == '3'): ?>护士<?php endif; ?>)</td>
                            <td><?php echo ($v["iphone"]); ?></td>
                            <td><?php echo ($v["num"]); ?></td>
                            <td><?php if($v["star"] >= 5): ?><span style="color:green">好评</span><?php endif; ?>
                            <?php if($v["star"] < 3): ?><span style="color:red">差评</span><?php endif; ?>
                            <?php if(($v["star"] >= 3) AND ($v["star"] < 5)): ?><span style="color:orange">中评</span><?php endif; ?></td>
                            <!-- <td><?php echo (date("Y-m-d H:i:s",$v["inputtime"])); ?></td>
                            <td><?php echo ($v['status']?"<span style='color:green'>已显示</span>":"<span style='color:red'>已隐藏</span>"); ?></td> -->
                            <td>
                                <div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">
                                    <a class="green" href="/index.php/Admin/Comment/info/muserid/<?php echo ($v["muserid"]); ?>"> <i class="icon-eye-open bigger-130"></i>&nbsp;查看评论 </a>
                                </div>
                            </td>
