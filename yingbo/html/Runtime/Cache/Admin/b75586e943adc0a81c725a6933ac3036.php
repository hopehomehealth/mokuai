        ...
        <div class="row">
            <div class="page-content box">
                <div class="box-title margin_bot_20">
                    <div class="span10">
                        <h3><i class="icon-cogs"></i>提现管理</h3>
                    </div>
                </div>
                <form class="form-horizontal" action="<?php echo U('Admin/Postal/index');?>" method="post">
                    <div class="form-group">
                        <label class="control-label" style="width:80px;">提现单号 : </label>
                ...
                <table class="table table-striped table-bordered table-hover">
                    <thead>
                    <tr><th width="50px" >提现ID</th>
                        <th width="50px">提现单号</th>
                        <th width="100px">提现金额</th>
                        <th width="60px">申请时间</th>
                        <th width="60px">成功时间</th>
                        <th width="60px">状态</th>
                        <th width="100px">操作</th>
                    </tr></thead>
                    <tbody>
                    <?php if(is_array($postalList)): foreach($postalList as $k=>$v): ?><tr>
                            <td><?php echo ($v["count_id"]); ?></td>
                            <td><?php echo ($v["order_no"]); ?></td>
                            <td><?php echo ($v["balance"]); ?></td>
                            <td><?php echo (date("Y-m-d H:i:s",$v["inputtime"])); ?></td>
                            <td><?php if(!empty($v["succtime"])): echo (date("Y-m-d H:i:s",$v["succtime"])); endif; ?></td>
                            <?php if($v["status"] == 1): ?><td><span style="color:green">已提现</span></td>
                            <?php else: ?>
                            <td><span style="color:red">待处理</span></td><?php endif; ?>
                            <td>
                                <div class="visible-md visible-lg hidden-sm hidden-xs action-buttons">
                                    <a class="green" href="<?php echo U('examine',array('cash_id'=>$v['cash_id'],'userid'=>$v['userid'],'count_id'=>$v['count_id'],'balance'=>$v['balance'],'status'=>$v['status'],'pre_money'=>$v['pre_money']));?>"> <i class="icon-pencil bigger-130"></i>&nbsp;审核 </a>
