...
function getSmsVerify(act,btn,mobile){
    ...
    var D = { act:act, mobile:mobile, scode:scode, username:username, password:password };
    $.post("/welcome/sms", D,
        function(data){
            var type = 8;
            if(data.error==0){
...
function addToCart(id,type,obj,pre){
    ...
    $.post('/yunbuy/addtocart',{id:id,qty:qty},function(data){
        if(data.error){
            var params = ' ';
            if(data.url){
                params = function(){
