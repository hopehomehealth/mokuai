...
class captcha{
    ...
    * @return void
      +----------------------------------------------------------
    */
    static function generate($len=4,$font_size=48,$name='icode',$width='',$height=''){
        if($width=='') $width=($font_size+5)*($len+1);
        if($height=='') $height=($font_size)*2;
        $fonts = array(
            'font' => 'bcdefhkmnrstuvwxyABCDEFGHKMNPRSTUVWXY345689',
            'cartouche' => 'bcdefhkmnrstuvwxyABCDEFGHKMNPRSTUVWXY345689',
        );
        ...
        for($num=0;$num<10;$num++){
            ...
            for($i=0;$i<$len;$i++){
                imagettftext($image,$font_size,rand(-3,3),$font_size/2+($font_size+5)*$i,$height/1.25-rand(2,3),$text_color,AppDir.'includes/captcha/'.$font.'.ttf',$str[$i]);//用规定字体向图像写入文本
            }
            imagegif($image);
            imagedestroy($image);
