...
class RSA
{
	...
	public function __construct($conf)
	{
		...
		$this->_privFile = AppDir . 'includes/modules/payment/'.$items->item(0)->getElementsByTagName('pfx')->item(0)->nodeValue;
		$this->_pubFile = AppDir . 'includes/modules/payment/'.$items->item(0)->getElementsByTagName('cer')->item(0)->nodeValue;
		$this->_algo = OPENSSL_ALGO_SHA1;
        $payment = get_payment('jubaopay');
		$this->_psw = $payment['psw'];
	}
