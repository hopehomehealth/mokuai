...
class TeegonService{
    ...
    function pay($param,$result_decode = true){
        ...
        $param['client_id'] = $param['TEE_CLIENT_ID'];
        $param['client_secret'] = $param['TEE_CLIENT_SECRET'];
    ...
    function verify_return(){
        if($_GET['charge_id']){
            ...
            return array('status'=>"3",'error_msg'=>'<h1>参数错误!</h1><hr ><pre>','param'=>$_GET);
        }
    }
    ...
    public function sign($para_temp,$payment){
        ...
        //生成加密字符串
        $prestr = $this->create_string($para_sort);
        $prestr = $payment['TEE_CLIENT_SECRET'] .$prestr . $payment['TEE_CLIENT_SECRET'];
    ...
    private function para_filter($para) {
        ...
        while (list ($key, $val) = each ($para)) {
            if($key == "sign"||$key=="/welcome/respond")continue;
