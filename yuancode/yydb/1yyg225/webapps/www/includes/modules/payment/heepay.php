...
$payment_lang = AppDir . 'includes/languages/payment/heepay.php';
...
class heepay {
    ...
    public function __construct() {
        $payment = get_payment('heepay');
        //商户号
        $this->agentId = $payment['app_id'];
        //签名密钥
        $this->signKey = $payment['app_key'];
        $this->returnUrl = url('/welcome/respond') . '?code=heepay'; //回调地址
    }
    ...
    public function get_code($order, $payment) {
        ...
        $url = '';
        $url .= '<form id="frmSubmit" method="post" name="frmSubmit" action="' . $this->payUrl . '">';
    ...
    public function respond() {
        ...
        $signStr = $signStr . '&agent_id=' . $agentId;
        $signStr = $signStr . '&jnet_bill_no=' . $jnetBillNo;
        $signStr = $signStr . '&agent_bill_id=' . $agentBillId;
        $signStr = $signStr . '&pay_type=' . $payType;
        $signStr = $signStr . '&pay_amt=' . $payAmt;
        ...
        if ($sign == $returnSign && $result == 1) {   //比较MD5签名结果 是否相等 确定交易是否成功  成功返回ok 否则返回error$remark1
