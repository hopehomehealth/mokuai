<?php
$GLOBALS['key'] = $payment['shanpay_key'];
 ...
 *
 */
function buildRequestFormShan($para_temp, $button='') {
	//待请求参数数组
	$para = buildRequestParaShan($para_temp);
    ...
    $sHtml = $sHtml."<input type='submit' class='payBtn' value='".$button."' /></form>";
 ...
 * @return 要请求的参数数组
 */
function buildRequestParaShan($para_temp) {
	//除去待签名参数数组中的空值和签名参数
	$para_filter = paraFilterShan($para_temp);
	//对待签名参数数组排序
	$para_sort = argSortShan($para_filter);
	//生成签名结果
	$mysign = buildRequestMysignShan($para_sort);
 ...
 * return 签名结果字符串
 */
function buildRequestMysignShan($para_sort) {
	//把数组所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串
	$prestr = createLinkstringShan($para_sort);
	$mysign = md5SignShan($prestr, $GLOBALS['key']);
	return $mysign;
}
