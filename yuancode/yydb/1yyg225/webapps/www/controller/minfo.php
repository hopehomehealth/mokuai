...
class minfo extends Lowxp{
    ...
    function index(){
        $id = intval($_GET['id']);
        if(empty($id)){ $this->msg();die; }
        $row['title'] = '个人空间';

        $member = $this->member->member_info($id);
        /*if($member['status']==0){
            $this->msg('该帐号因违规已封号！',array(
                'iniframe' => false,
                'url' => url()
            ));die;
        }*/
        $member['intro'] = $member['intro'] ? $member['intro'] : '这家伙忙着'.$this->L['unit_unit'].'，还没写签名呢。';
        ...
        $this->smarty->assign('isfri', $isfri);
        ...
        if(STPL == 'mobile'){
            ...
            //访客查询
            $sql = "SELECT * FROM ###_member_visit WHERE mid1=$id ORDER BY lasttime DESC LIMIT 15";
            $data['visit'] = $this->db->select($sql);
            $data['visit'] = $this->db->lJoin($data['visit'],'member','mid,photo,username,nickname','mid2','mid');

            //好友查询
            $sql = "SELECT * FROM ###_member_fri WHERE mid2=$id ORDER BY c_time DESC LIMIT 6";
            $data['fri'] = $this->db->select($sql);
            $data['fri'] = $this->db->lJoin($data['fri'],'member','mid,photo,username,nickname','mid1','mid');
            $sql = "SELECT COUNT(mid1) FROM ###_member_fri WHERE mid1=$id";
            $data['fri_count'] = $this->db->getstr($sql);
        }
        $this->display_before($row);
        $this->smarty->assign('data',$data);
        $this->smarty->display('minfo/index.html');
    }
    //ajax加载夺宝记录
    function load_db($id,$page=1){
        ...
        if($vid){
            ...
            $this->smarty->assign('db_info',$info);
            $this->smarty->assign('member_id',$_SESSION['mid']);
            $this->smarty->display('minfo/load_db_vid.html');
        }else{
            $size = 10;
            ...
            $this->smarty->assign('list',$list);
            $this->smarty->display('minfo/load_db.html');
        }
    }
    //ajax加载夺宝中奖
    function load_dbCod($id,$page=1){
        ...
        $this->smarty->assign('list',$list);
        $this->smarty->display('minfo/load_dbCod.html');
    }
    //ajax加载竞拍记录
    ...
    function load_share($id,$page=1){
        ...
        if($vid){
            ...
            if($share['extension_code']==CART_DB){
                ...
                $this->smarty->assign("ended_db",$ended_db);
            }
            ...
            $this->smarty->assign('new_share',$new_share);
            $this->smarty->display('minfo/load_share_vid.html');
        }else{
            $size = 18;
            $list = $this->yunbuy->getShare(" s.mid = '$id' ORDER BY addtime DESC",$size,$page,"s.*",'href="/minfo/share/{num}?id='.$id.'"',$id.'/');
            $this->smarty->assign('list',$list);
            $this->smarty->display('minfo/load_share.html');
        }
    }
    //ajax加载访客
