...
class RegisterController extends CommonController {
    ...
    public function find_pwd(){
        if (IS_POST) {
            ...
            }elseif(session('short_mobile')!=$mobile){
                $this->returnMessage(0,'手机号码不一致','');
            } elseif (empty(session('short_msg_code'))) {
                $this->returnMessage(0, '验证码已过期', '');
            } elseif (session('short_msg_code') != $code) {
                $this->returnMessage(0, '验证码不正确', '');
            } else {
...
public function resetPassword(){
    ...
    public function register(){
        if(IS_POST){
            ...
            if(empty(session('short_msg_code'))){$this->returnMessage(0,'验证码已过期','');}
            if(session('short_mobile')!=$mobile){$this->returnMessage(0,'手机号码不一致','');}
            if(session('short_msg_code')!=$verify){$this->returnMessage(0,'验证码不正确','');}
            $data=array(
                    'user_name'=>$user_name,
                    'mobile'=>$mobile,
                    'email'=>$email,
