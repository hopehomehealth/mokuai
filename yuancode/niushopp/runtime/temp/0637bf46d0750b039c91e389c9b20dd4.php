<?php if (!defined('THINK_PATH')) exit(); /*a:6:{s:37:"template/adminblue\Auth\userList.html";i:1509355633;s:28:"template/adminblue\base.html";i:1512444889;s:45:"template/adminblue\controlCommonVariable.html";i:1506689787;s:32:"template/adminblue\urlModel.html";i:1510819828;s:34:"template/adminblue\pageCommon.html";i:1512444925;s:34:"template/adminblue\openDialog.html";i:1509523953;}*/ ?>
<!DOCTYPE html>
<html>
	<head>
...
<table class="mytable">
	<tr>
		<th style="text-align: left;"><button class="btn-common btn-small" onclick="location.href='<?php echo __URL('ADMIN_MAIN/auth/adduser'); ?>';" style="margin:0 !important;">添加用户</button></th>
		<th>
...
function LoadingInfo(page_index) {
	...
	$.ajax({
		type : "post",
		url : "<?php echo __URL('ADMIN_MAIN/auth/userlist'); ?>",
	...
	$("#dialog").dialog({
		buttons: {
			"确定,#e57373": function() {
				...
				$.ajax({
					type : "post",
					url : "<?php echo __URL('ADMIN_MAIN/auth/resetuserpassword'); ?>",
					data : { "uid" : uid },
					success : function(data) {
						if(data["code"] > 0 ){
							showMessage('success', data["message"],"<?php echo __URL('ADMIN_MAIN/auth/userlist'); ?>");
						}else{
...
function lock(uid){
	$.ajax({
		type : "post",
		url : "<?php echo __URL('ADMIN_MAIN/auth/userlock'); ?>",
		data : { "uid" : uid },
		success : function(data) {
			if(data["code"] > 0 ){
				showMessage('success', data["message"],"<?php echo __URL('ADMIN_MAIN/auth/userlist'); ?>");
...
function unlock(uid){
	$.ajax({
		type : "post",
		url : "<?php echo __URL('ADMIN_MAIN/Auth/userunlock'); ?>",
		data : { "uid" : uid },
		success : function(data) {
			if(data["code"] > 0 ){
				showMessage('success', data["message"],"<?php echo __URL('ADMIN_MAIN/auth/userlist'); ?>");
	...
	$( "#dialog" ).dialog({
		buttons: {
			"确定": function() {
				$(this).dialog('close');
				$.ajax({
				url : "<?php echo __URL('ADMIN_MAIN/auth/deleteadminuserajax'); ?>",
				data : {"uid" : uid },
				type : "post",
				dataType : "json",
				success : function(data) {
